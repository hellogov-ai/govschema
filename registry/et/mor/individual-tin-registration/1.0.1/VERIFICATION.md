# Verification record — et/mor/individual-tin-registration@1.0.1

## PATCH 1.0.0 → 1.0.1

GOV-4250 (GOV-4249 review gate follow-up). The 1.0.0 record claimed this
schema models 34 `fields[]`; the actual `fields[]` array has 38 entries
(confirmed via `node -e "require('./schema.json').fields.length"` → 38, and
independently re-confirmed during the GOV-4249 review gate's own
pdfjs-dist re-extraction of all 4 pages). Purely a prose correction to a
summary count — no field, validation, or structural change of any kind —
so this ships as a PATCH per VERSIONING.md's immutability rule rather than
editing the published 1.0.0 directory. Every other detail below is
otherwise identical to 1.0.0's own record.
...
Models 38 `fields[]` across 6 steps (Registration & Occupation; Personal
...
`conformance/et/mor/individual-tin-registration/1.0.1/`:
...
(587/587 both), individually and as part of the full registry run.
`registry-index.json` regenerated via `npm run build-index` in
`tools/govschema-client/` (586 → 587 entries).
