# Verification record — mt/transport-malta/driving-licence-renewal@1.0.0
...
- Confirmed mechanically: the retrieved bytes begin `%PDF-1.4` and contain
...
`conformance/mt/transport-malta/driving-licence-renewal/1.0.0/`:
...
(584/584 both), individually and as part of the full registry run.
`registry-index.json` regenerated via `npm run build-index` in
`tools/govschema-client/` (583 → 584 entries).
