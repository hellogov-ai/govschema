...
| **National ID & Civic Documents** | 12/13 (92%) | none in the existing 12 — closed (SG voter-reg is a confirmed non-gap); **KR** not yet modelled (candidate: `GOV-1294`) |
...
### National ID & Civic Documents (12/13 jurisdictions) — closed for the original 12

Every jurisdiction except South Korea has at least one National ID and/or
voter-registration schema. Singapore's lack of a voter-registration schema is
a **confirmed non-gap** (GOV-1075): Singapore voting is compulsory and
NRIC-linked, with no citizen-initiated online registration step to model.
**KR** is not yet modelled — GOV-1289's research found two strong candidates
(Resident Registration Card reissuance form, and NEC's overseas-voter
registration form with an official filled-in example), filed as `GOV-1294`.
...
- **Scraping:** `find registry -mindepth 3 -maxdepth 3 -type d` (one entry
  ...
  `tools/govschema-client/registry-index.json` (213 entries, one per
