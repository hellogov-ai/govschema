...
> средство" (Приложение № 2а — Application for Issuance of a Driving Licence).
> This candidate was pre-scouted and confirmed genuine during the prior
> GOV-2837 cycle (which corrected an earlier, too-broad GOV-2830 claim that
> "Bulgaria's DMV" was a dead end — that dead end applies only to MVR's
> vehicle-registration application, a distinct, system-generated-at-the-
> counter form with no independently-fillable blank) and authored this cycle,
> re-verified from scratch rather than trusted from the prior cycle's own
> numbers: fetched directly from `mvr.bg`'s own "Бланки и образци"
> static-forms page (HTTP 200, `application/pdf`, 382,212 bytes,
> sha256 `e633ee22c10e650186b69b5c2278a51c02eb8fb7c413fee875c724066cec9576`),
> confirmed byte-identical across two independent fetch methods (`curl` and
> `wget`, both against the live origin). `pdfjs-dist` confirmed a genuine
> one-page, zero-AcroForm-widget text-layer specimen; a full-page render at
> 2.5x scale cross-checked reading order and cell boundaries the raw
> text-layer's y-coordinates alone left ambiguous. Models 29 `fields[]`
> (requested service tier, national ID number, date/place of birth, name in
> both Cyrillic and Latin script plus a separate family-name/other-names
> breakdown, address, sex, other nationality/naturalization-decree
> particulars, contact details, receiving office, an optional
> authorized-person/proxy block, an optional driving-licence-category-decline
> declaration, the identity document presented at submission, submission
> date, and signature) plus 3 `documents[]` entries (photograph, signature
> specimen, and the form's own criminal-liability truthfulness declaration).
> Excludes six "Вярно/Невярно" (True/False) checkbox pairs tied to a
> system-assisted counter workflow (civil-registry data auto-fill plus
> confirm/dispute) rather than the plain paper-form completion this schema
> models, plus the office-intake "Вх. номер"/"До" header fields and the
> receiving official's own name/signature line. A prior agent session's
> worktree for this issue was found to already contain a substantially
> complete, uncommitted draft (an infra-crash artifact); it was independently
> re-verified from scratch and one genuine defect was found and fixed — the
> draft's own description undercounted the confirm/dispute checkboxes at five
> where a direct re-count confirms six. See GOV-2853 and this schema's own
> VERIFICATION.md for the full sourcing record. **Bulgaria now stands at 3 of
> 6 verticals** (Taxes ×2, Visa, DMV); Passport and National ID & Civic
> Documents remain open backlog.
...
> `bg/mvnr/zayavlenie-za-izdavane-na-natsionalna-viza-tip-d` — the
> Ministerstvo na vanshnite raboti's (MVnR, Bulgaria's Ministry of Foreign
> Affairs) "Заявление за издаване на национална виза (тип „D”)" (Application
> for Issuance of a National Visa, Type "D"), the long-stay national visa
> application filed at a Bulgarian diplomatic/consular mission for stays
> exceeding 90 days. This cycle screened DMV and Business Formation first
> (per the cycle's own priority ranking) and found both genuine dead ends:
> `mvr.bg`'s own vehicle-registration service page states the application "is
> generated by MVR's information system" and is not independently fillable
> in advance (the same system-generated-at-the-counter pattern as Bulgaria's
> already-screened National ID); and `portal.registryagency.bg`'s Commercial
> Register document-templates page renders its template-file links via
> client-side JavaScript with no static first-party attachment reachable by
> an unauthenticated fetch (the same portal-shell pattern GOV-2821 found for
> NRA's self-employed-registration lead). The Visa candidate, by contrast,
> was fetched directly from `mfa.bg`'s own "Формуляри за виза" consular-
> services page as a free, unauthenticated "Безплатен образец" PDF download
> (HTTP 200, `application/pdf`, 144,794 bytes, sha256
> `427738994b1e3a87ff1d6fa35d1ef8ad2678464d70bb70ac922eb794cfcff932`) — a
> genuine text-layer PDF (zero fillable AcroForm widgets) closely following
> the EU-harmonized visa-application layout this registry already models for
> several other jurisdictions. Models 108 `fields[]` plus 2 `documents[]`
> entries: applicant/travel-document identification, a marital-status block
> with a conditionally-required spouse/registered-partner sub-block, a
> bounded 4-row children table and a bounded 3-row previous-Bulgaria-stays
> table (both row counts confirmed by rendering the source's own pages, not
> guessed), prior-refusal/conviction/expulsion/notifiable-disease
> declarations each gating its own "if yes" detail field per the form's own
> printed instruction, and a two-sided (applicant/sponsor) funding-source
> checklist. Excludes the form's entire "Попълва се служебно" (staff-only)
> column throughout, consistent with this registry's established consular-
> only-column precedent. **Bulgaria now stands at 2 of 6 verticals**
> (Taxes, Visa); DMV and Business Formation are now confirmed dead ends for
> this cycle's sourcing attempt (re-attempt Business Formation with a
> JavaScript-executing browser in a future cycle); Passport and National ID
> (already spot-checked weak) remain open backlog. See GOV-2830 and this
...
"Заявление за издаване на свидетелство за управление на моторно превозно
средство" (Приложение № 2а, Application for Issuance of a Driving Licence).
See the Executive Summary's GOV-2853 update above and the document's own
VERIFICATION.md for the full sourcing record, including the corrected
...
Foreign Affairs' (MVnR) "Заявление за издаване на национална виза (тип „D”)"
(Application for Issuance of a National Visa, Type "D"), the long-stay
national visa application filed at a Bulgarian diplomatic/consular mission
for stays exceeding 90 days. DMV (МВР — system-generated at the counter, not
independently fillable) and Business Formation (Registry Agency's
Commercial Register templates — client-side-JavaScript-gated, no static
first-party attachment reachable) were both screened first this cycle and
found to be genuine dead ends. Fetched directly and unauthenticated from
`mfa.bg`'s own "Формуляри за виза" page (HTTP 200, `application/pdf`,
144,794 bytes, `sha256:
427738994b1e3a87ff1d6fa35d1ef8ad2678464d70bb70ac922eb794cfcff932`). A flat,
non-AcroForm 4-page specimen (zero fillable Widget annotations, confirmed
via `pdfjs-dist`) with a genuine, fully extractable Bulgarian text layer;
all four pages were rendered to PNG and visually cross-checked, confirming
the exact printed row counts for two tabular sections (a 4-row children
table and a 3-row previous-Bulgaria-stays table, both modeled as bounded
repeating groups) and that the entire right-hand "Попълва се служебно"
(staff-only) column throughout pages 1-2 is consular-adjudication-only
(excluded). Models 108 `fields[]` plus 2 `documents[]` entries, including a
conditionally-required spouse/registered-partner block gated on marital
status, and four Yes/No declarations (prior refusal/conviction/expulsion/
notifiable disease) each gating its own "if yes" detail field per the
form's own printed instruction. See the Executive Summary's GOV-2830 update
above and the document's own VERIFICATION.md for the full sourcing record.
Bulgaria now stands at 2 of 6 verticals (Taxes, Visa); Passport and National
ID (already spot-checked weak) remain open backlog candidates.
...
- **Bulgaria — Visa: authored (GOV-2830), Bulgaria now stands at 2 of 6
  verticals.** `bg/mvnr/zayavlenie-za-izdavane-na-natsionalna-viza-tip-d`
  (MVnR's national "Type D" long-stay visa application) opened Bulgaria's
  Visa vertical — see the Executive Summary's GOV-2830 update. This cycle
  also re-screened DMV and Business Formation and confirmed both are dead
  ends for a first-party unauthenticated fetch: `mvr.bg`'s own vehicle-
  registration page states the application is system-generated at the
  counter (not independently fillable), and `portal.registryagency.bg`'s
  Commercial Register template page gates its actual template files behind
  client-side JavaScript with no static attachment reachable (the same
  portal-shell pattern as the earlier NRA self-employed-registration lead).
  **Bulgaria's Passport and National ID verticals remain open backlog** —
  ...
  **Correction (GOV-2837): Bulgaria's DMV dead end was scoped only to
  ...
  cycle.
- **Authored (GOV-2853): Bulgaria's DMV vertical opens (3 of 6), superseding
  the open-backlog note immediately above.** The driving-licence-issuance
  candidate identified in the GOV-2837 correction was authored this cycle as
  `bg/mvr/zayavlenie-za-izdavane-na-svidetelstvo-za-upravlenie-na-mps` — see
  the Executive Summary's GOV-2853 update and the document's own
